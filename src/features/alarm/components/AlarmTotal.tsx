...
import AlarmComponent from '@/features/alarm/components/AlarmComponent';
import Trip from '@/features/alarm/components/Trip';
...
export default function AlarmTotal() {
  ...
  setActivePicker(null);
	...
	return (
		<div className="bg-grey-10 min-h-screen">
				...
				<div className="relative flex items-center justify-between w-full bg-white py-3">
					...
					<Icon name={'ArrowLeftBar'} className="text-grey-70 mx-4" size="xxl"/>
					</button>
    ...
    className={`flex-1 py-2 font-semibold ${
      pickerTab === 'store' ? 'text-primary-500 border-b-2 border-primary-500' : 'text-grey-60'
    }`}
  ...
  <button
    className={`flex-1 py-2 font-semibold ${
      pickerTab === 'review' ? 'text-primary-500 border-b-2 border-primary-500' : 'text-grey-60'
    }`}
    ...
    리뷰
          ...
          {years.map((y) => (
            <Picker.Item key={y} value={y}>
				{({selected}) => (
					<span className={selected ? "text-primary-40 font-bold" : "text-black"}>
						{y}
					</span>
				)}
            </Picker.Item>
		...
		<Picker.Column name="yearSuffix">
          ...
          {months.map((m) => (
            <Picker.Item key={m} value={m}>
              {({selected}) => (
				<span className={selected ? "text-primary-40 font-bold ml-2" : "text-black"}>
					{m}
				</span>
			  )}
            </Picker.Item>
    ...
    <>
	  ...
	  <span className="text-black">지</span>
					...
					<div className="mt-4 text-grey-90 text-[14px] font-semibold">
						{activePicker === 'start' && (
							<span>{pickerValue.year}.{pickerValue.month.padStart(2,"0")} ~</span>
						)}
						{activePicker === 'end' && (
							<span>{startDate} ~ {pickerValue.year}.{pickerValue.month.padStart(2, "0")}</span>
						)}
					</div>
